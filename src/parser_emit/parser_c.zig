...
fn writeGlrVersionStorage(writer: anytype, has_external_scanner: bool) !void {
    ...
    try writer.writeAll("  uint16_t field_map_index;\n");
    try writer.writeAll("  uint16_t field_map_length;\n");
    try writer.writeAll("  uint16_t children[GEN_Z_SITTER_MAX_NODE_CHILDREN];\n");
    try writer.writeAll("  TSSymbol child_aliases[GEN_Z_SITTER_MAX_NODE_CHILDREN];\n");
    ...
    try writer.writeAll("  uint32_t error_cost;\n");
    try writer.writeAll("  uint32_t recovery_attempts;\n");
    try writer.writeAll("  uint32_t stack_recoveries;\n");
    try writer.writeAll("  uint32_t skipped_tokens;\n");
    try writer.writeAll("  uint32_t skipped_bytes;\n");
    ...
    try writer.writeAll("    set->versions[i].error_cost = 0;\n");
    try writer.writeAll("    set->versions[i].recovery_attempts = 0;\n");
    try writer.writeAll("    set->versions[i].stack_recoveries = 0;\n");
    try writer.writeAll("    set->versions[i].skipped_tokens = 0;\n");
    try writer.writeAll("    set->versions[i].skipped_bytes = 0;\n");
    ...
    try writer.writeAll("  return true;\n");
    try writer.writeAll("}\n\n");
    try writer.writeAll("static bool ts_generated_version_reduce_value(TSGeneratedParseVersion *version, const TSParseAction *action, const TSSymbol *alias_sequence, TSMapSlice field_map) {\n");
    ...
    try writer.writeAll("    .field_map_index = field_map.index,\n");
    try writer.writeAll("    .field_map_length = field_map.length,\n");
    ...
    try writer.writeAll("    version->nodes[node_id].child_aliases[child_index] = alias_sequence[child_index];\n");
    try writer.writeAll("  }\n");
    ...
    try writer.writeAll("    .field_map_index = 0,\n");
    try writer.writeAll("    .field_map_length = 0,\n");
    try writer.writeAll("  }, &node_id)) return false;\n");
    try writer.writeAll("  return ts_generated_version_push_value(version, (TSGeneratedValue){\n");
    try writer.writeAll("    .symbol = symbol,\n");
    try writer.writeAll("    .start_byte = version->byte_offset,\n");
...
fn writeGlrActionDispatch(writer: anytype) !void {
    ...
    try writer.writeAll("      if (!ts_generated_version_reduce_value(version, action, &ts_alias_sequences[action->reduce.production_id][0], ts_field_map_slices[action->reduce.production_id])) break;\n");
    ...
    try writer.writeAll("    version->error_cost++;\n");
    try writer.writeAll("    version->stack_recoveries++;\n");
    try writer.writeAll("    return true;\n");
    try writer.writeAll("  }\n");
    try writer.writeAll("  return false;\n");
    try writer.writeAll("}\n\n");
    try writer.writeAll("static TSGeneratedParseStep ts_generated_drive_version(TSGeneratedParseVersion *version, TSSymbol lookahead_symbol, uint32_t advance_bytes) {\n");
    ...
    try writer.writeAll("      version->recovery_attempts++;\n");
    ...
    try writer.writeAll("        version->byte_offset += advance_bytes;\n");
    try writer.writeAll("        version->error_cost += advance_bytes;\n");
    try writer.writeAll("        version->skipped_tokens++;\n");
    try writer.writeAll("      } else {\n");
    try writer.writeAll("        version->byte_offset++;\n");
    try writer.writeAll("        version->error_cost++;\n");
    try writer.writeAll("        version->skipped_bytes++;\n");
    try writer.writeAll("      }\n");
    try writer.writeAll("      version->error_count++;\n");
    try writer.writeAll("      step.status = TSGeneratedParseStepRecover;\n");
    try writer.writeAll("      return step;\n");
...
fn writeGlrUnresolvedForkHelpers(writer: anytype) !void {
    ...
    try writer.writeAll("      }\n");
    try writer.writeAll("      if (step.status == TSGeneratedParseStepReduce) (void)ts_generated_drive_version(&set->versions[target_index], lookahead_symbol, advance_bytes);\n");
...
fn writeGlrVersionCondenseHelpers(writer: anytype, has_external_scanner: bool) !void {
    ...
    try writer.writeAll("    if (left->nodes[index].field_map_index != right->nodes[index].field_map_index) return false;\n");
    try writer.writeAll("    if (left->nodes[index].field_map_length != right->nodes[index].field_map_length) return false;\n");
    ...
    try writer.writeAll("  uint32_t min_error_cost = UINT32_MAX;\n");
    try writer.writeAll("  for (uint16_t scan_index = 0; scan_index < set->count; scan_index++) {\n");
    try writer.writeAll("    if (set->versions[scan_index].active && set->versions[scan_index].error_cost < min_error_cost) {\n");
    try writer.writeAll("      min_error_cost = set->versions[scan_index].error_cost;\n");
    try writer.writeAll("    }\n");
    try writer.writeAll("  }\n");
    ...
    try writer.writeAll("        if (set->versions[read_index].error_cost < set->versions[existing_index].error_cost) {\n");
    try writer.writeAll("          set->versions[existing_index] = set->versions[read_index];\n");
    try writer.writeAll("        } else if (set->versions[read_index].error_cost == set->versions[existing_index].error_cost && set->versions[read_index].dynamic_precedence > set->versions[existing_index].dynamic_precedence) {\n");
...
fn writeGlrVersionStepLoop(writer: anytype, has_unresolved: bool) !void {
    ...
    try writer.writeAll("    TSGeneratedParseStep step = ts_generated_drive_version(version, lookahead_symbol, advance_bytes);\n");
...
fn writeGlrMainParseFunction(writer: anytype) !void {
    ...
    try writer.writeAll("      result.error_cost = lead->error_cost;\n");
    try writer.writeAll("      result.recovery_attempts = lead->recovery_attempts;\n");
    try writer.writeAll("      result.stack_recoveries = lead->stack_recoveries;\n");
    try writer.writeAll("      result.skipped_tokens = lead->skipped_tokens;\n");
    try writer.writeAll("      result.skipped_bytes = lead->skipped_bytes;\n");
    ...
    try writer.writeAll("}\n\n");
    try writer.writeAll("static bool ts_generated_append_tree_string(char *buffer, uint32_t capacity, uint32_t *length, const char *text) {\n");
    try writer.writeAll("  while (*text) {\n");
    try writer.writeAll("    if (*length + 1 >= capacity) return false;\n");
    try writer.writeAll("    buffer[(*length)++] = *text++;\n");
    try writer.writeAll("  }\n");
    try writer.writeAll("  buffer[*length] = 0;\n");
    try writer.writeAll("  return true;\n");
    try writer.writeAll("}\n\n");
    try writer.writeAll("static bool ts_generated_write_node_string(const TSGeneratedParseResult *result, uint16_t node_id, TSSymbol alias_symbol, char *buffer, uint32_t capacity, uint32_t *length) {\n");
    try writer.writeAll("  if (node_id == GEN_Z_SITTER_NO_NODE || node_id >= result->node_count) return true;\n");
    try writer.writeAll("  const TSGeneratedNode *node = &result->nodes[node_id];\n");
    try writer.writeAll("  TSSymbol symbol = alias_symbol != 0 ? alias_symbol : node->symbol;\n");
    try writer.writeAll("  if (!ts_symbol_metadata[symbol].named) return true;\n");
    try writer.writeAll("  if (!ts_generated_append_tree_string(buffer, capacity, length, \"(\")) return false;\n");
    try writer.writeAll("  if (!ts_generated_append_tree_string(buffer, capacity, length, ts_symbol_names[symbol])) return false;\n");
    try writer.writeAll("  for (uint16_t child_index = 0; child_index < node->child_count; child_index++) {\n");
    try writer.writeAll("    uint16_t child_id = node->children[child_index];\n");
    try writer.writeAll("    if (child_id == GEN_Z_SITTER_NO_NODE || child_id >= result->node_count) continue;\n");
    try writer.writeAll("    TSSymbol child_symbol = result->nodes[child_id].symbol;\n");
    try writer.writeAll("    if (!ts_symbol_metadata[child_symbol].named) continue;\n");
    try writer.writeAll("    if (!ts_generated_append_tree_string(buffer, capacity, length, \" \")) return false;\n");
    try writer.writeAll("    if (!ts_generated_write_node_string(result, child_id, 0, buffer, capacity, length)) return false;\n");
    try writer.writeAll("  }\n");
    try writer.writeAll("  return ts_generated_append_tree_string(buffer, capacity, length, \")\");\n");
    try writer.writeAll("}\n\n");
    try writer.writeAll("bool ts_generated_result_tree_string(const TSGeneratedParseResult *result, char *buffer, uint32_t capacity) {\n");
    try writer.writeAll("  if (!result || !buffer || capacity == 0) return false;\n");
    try writer.writeAll("  buffer[0] = 0;\n");
    try writer.writeAll("  if (!result->accepted || result->root_node == GEN_Z_SITTER_NO_NODE) return false;\n");
    try writer.writeAll("  uint32_t length = 0;\n");
    try writer.writeAll("  return ts_generated_write_node_string(result, result->root_node, 0, buffer, capacity, &length);\n");
    try writer.writeAll("}\n\n");
}
...
test "emitParserCAlloc emits opt-in GLR parser version storage" {
    ...
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "#define GEN_Z_SITTER_MAX_NODE_CHILDREN 16\n\n"));
    ...
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "  uint16_t children[GEN_Z_SITTER_MAX_NODE_CHILDREN];\n  TSSymbol child_aliases[GEN_Z_SITTER_MAX_NODE_CHILDREN];\n} TSGeneratedNode;\n"));
    ...
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "  uint32_t error_count;\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "  uint32_t error_cost;\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "  uint32_t recovery_attempts;\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "  uint32_t stack_recoveries;\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "  uint32_t skipped_tokens;\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "  uint32_t skipped_bytes;\n"));
    ...
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "static bool ts_generated_version_reduce_value(TSGeneratedParseVersion *version, const TSParseAction *action, const TSSymbol *alias_sequence, TSMapSlice field_map) {\n"));
...
test "emitParserCAlloc emits opt-in GLR action dispatch helpers" {
    ...
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "if (!ts_generated_version_reduce_value(version, action, &ts_alias_sequences[action->reduce.production_id][0], ts_field_map_slices[action->reduce.production_id])) break;\n"));
    ...
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "version->error_cost++;\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "version->stack_recoveries++;\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "static TSGeneratedParseStep ts_generated_drive_version(TSGeneratedParseVersion *version, TSSymbol lookahead_symbol, uint32_t advance_bytes) {\n"));
    ...
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "version->recovery_attempts++;\n"));
    ...
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "version->byte_offset += advance_bytes;\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "version->error_cost += advance_bytes;\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "version->skipped_tokens++;\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "version->error_cost++;\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "version->skipped_bytes++;\n"));
...
test "emitParserCAlloc emits opt-in GLR unresolved fork helpers" {
    ...
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "if (step.status == TSGeneratedParseStepReduce) (void)ts_generated_drive_version(&set->versions[target_index], lookahead_symbol, advance_bytes);\n"));
}
...
test "emitParserCAlloc emits opt-in GLR version condensation helpers" {
    ...
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "if (left->nodes[index].field_map_index != right->nodes[index].field_map_index) return false;\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "if (left->nodes[index].field_map_length != right->nodes[index].field_map_length) return false;\n"));
...
test "emitParserCAlloc emits opt-in GLR active-version step loop" {
    ...
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "TSGeneratedParseStep step = ts_generated_drive_version(version, lookahead_symbol, advance_bytes);\n"));
...
test "emitParserCAlloc emits opt-in GLR raw input parse entry point" {
    ...
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "result.error_cost = lead->error_cost;\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "result.recovery_attempts = lead->recovery_attempts;\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "result.stack_recoveries = lead->stack_recoveries;\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "result.skipped_tokens = lead->skipped_tokens;\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "result.skipped_bytes = lead->skipped_bytes;\n"));
    ...
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "bool ts_generated_result_tree_string(const TSGeneratedParseResult *result, char *buffer, uint32_t capacity) {\n"));
    try std.testing.expect(std.mem.containsAtLeast(u8, emitted, 1, "static bool ts_generated_write_node_string(const TSGeneratedParseResult *result, uint16_t node_id, TSSymbol alias_symbol, char *buffer, uint32_t capacity, uint32_t *length) {\n"));
