...
test "generateNodeTypesJsonFromPrepared matches the extra aliased body golden fixture" {
    var loader_arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer loader_arena.deinit();
    var parse_arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer parse_arena.deinit();
    ...
    var parsed = try std.json.parseFromSlice(
        std.json.Value,
        loader_arena.allocator(),
        fixtures.extraAliasedBodyGrammarJson().contents,
        .{},
    );
    defer parsed.deinit();
    ...
    try std.testing.expectEqualStrings(fixtures.extraAliasedBodyNodeTypesJson().contents, json);
}

test "generateNodeTypesJsonFromPrepared matches the external collision golden fixture" {
    var loader_arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer loader_arena.deinit();
    var parse_arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer parse_arena.deinit();
    ...
    var parsed = try std.json.parseFromSlice(
        std.json.Value,
        loader_arena.allocator(),
        fixtures.externalCollisionGrammarJson().contents,
        .{},
    );
    defer parsed.deinit();
    ...
    try std.testing.expectEqualStrings(fixtures.externalCollisionNodeTypesJson().contents, json);
}
