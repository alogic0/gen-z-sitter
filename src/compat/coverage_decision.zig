...
pub fn buildCoverageDecisionAlloc(
    ...
    const recommendation_rationale = if (deferred_parser_targets.len != 0)
        try duplicateStringSliceAlloc(allocator, &.{
            ...
            "one deferred parser-only fixture is an intentional upstream-rejected ambiguity, while JavaScript and Python are real external parser-only candidates held at load/prepare because broader parser-table proofs remain outside the bounded routine budget",
            "the next promoted milestone should narrow or promote the onboarded JavaScript and Python snapshots before adding another large parser-only grammar to the stable first wave",
        })
    else if (deferred_scanner_targets.len == 0)
        try duplicateStringSliceAlloc(allocator, &.{
            "the promoted first-wave parser-only shortlist currently passes within the staged boundary",
...
test "buildCoverageDecisionAlloc summarizes the current next-step decision" {
    ...
    try std.testing.expectEqual(@as(usize, 4), report.deferred_parser_only_targets.len);
