...
test "buildMismatchInventoryAlloc classifies the current shortlist" {
    ...
    try std.testing.expectEqual(@as(usize, 1), report.parser_only_incompatibilities.len);
...
test "renderMismatchInventoryAlloc matches the checked-in shortlist mismatch inventory artifact" {
    ...
    const expected = try std.Io.Dir.cwd().readFileAlloc(std.testing.io, "compat_targets/shortlist_mismatch_inventory.json", allocator, .limited(1024 * 1024));
