...
test "buildInventoryReportAlloc summarizes the shortlist boundary" {
    ...
    try std.testing.expectEqual(@as(usize, 14), report.boundary.total_shortlist_targets);
    ...
    try std.testing.expectEqual(@as(usize, 12), report.family_coverage.len);
    ...
    try std.testing.expectEqual(targets.TargetFamily.bracket_lang, report.family_coverage[9].family);
    try std.testing.expectEqual(@as(usize, 1), report.family_coverage[9].passed_count);
    try std.testing.expectEqual(targets.TargetFamily.hidden_external_fields, report.family_coverage[10].family);
    try std.testing.expectEqual(@as(usize, 2), report.family_coverage[10].passed_count);
    try std.testing.expectEqual(targets.TargetFamily.mixed_semantics, report.family_coverage[11].family);
