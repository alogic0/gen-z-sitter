...
test "buildExternalRepoInventoryAlloc summarizes the current external evidence" {
    ...
    try std.testing.expectEqual(@as(usize, 10), report.total_external_repo_targets);
    ...
    try std.testing.expectEqual(targets.TargetFamily.javascript, report.family_coverage[7].family);
    ...
    try std.testing.expectEqual(@as(usize, 10), report.targets.len);
}
...
test "renderExternalRepoInventoryAlloc matches the checked-in external repo inventory artifact" {
    ...
    const expected = try std.Io.Dir.cwd().readFileAlloc(std.testing.io, "compat_targets/external_repo_inventory.json", allocator, .limited(1024 * 1024));
