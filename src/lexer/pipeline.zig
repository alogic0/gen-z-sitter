...
pub const PipelineError =
    ...
    lexical_serialize.SerializeError ||
    lexical_dump.DebugDumpError ||
    std.json.ParseError(std.json.Scanner) ||
    std.mem.Allocator.Error;

pub fn generateSerializedLexicalDumpFromPrepared(
...
test "generateSerializedLexicalDumpFromPrepared matches the repeat choice seq lexical golden fixture" {
...
test "generateSerializedLexicalDumpFromPrepared matches the mixed semantics blocked lexical golden fixture" {
    var loader_arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer loader_arena.deinit();
    var parse_arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer parse_arena.deinit();
...
test "generateSerializedLexicalDumpFromPrepared keeps repeat choice seq lexical output stable across grammar.json and grammar.js" {
    var loader_arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer loader_arena.deinit();
    var parse_arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer parse_arena.deinit();
    ...
    const dump_from_json = try generateSerializedLexicalDumpFromPrepared(pipeline_arena.allocator(), prepared_from_json);
    ...
    const dump_from_js = try generateSerializedLexicalDumpFromPrepared(pipeline_arena_js.allocator(), prepared_from_js);
