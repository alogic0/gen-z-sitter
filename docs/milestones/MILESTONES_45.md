...
instead of the normal single-digit-second range. The fix was to avoid replacing
one whole-closure cache with repeated closure construction. Successor
construction now uses a shared `ClosureExpansionCache` for generated additions
and a seed-to-state cache for repeated successor seeds. That preserves the
important reuse without storing whole closed successor item sets.

Completed steps:

- [x] Add a shared closure-expansion cache owned by `ParseItemSetBuilder` or an
  adjacent item-set-construction context.
- [x] Reuse generated additions across successor closures.
- [x] Keep the existing full-closure cache until `tree_sitter_zig_json`
  serialize time stays in the previous range.
- [x] Then remove `ClosureResultCache` and rerun state/action regression checks.
